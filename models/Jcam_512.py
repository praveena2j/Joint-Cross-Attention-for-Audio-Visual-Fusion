...
class CAM(nn.Module):
    def __init__(self):
        ...
        self.W_ca = nn.Linear(1024, 32, bias=False)
        self.W_cv = nn.Linear(1024, 32, bias=False)
        ...
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        self.regressor = nn.Sequential(nn.Linear(1024, 128),
                                     nn.Dropout(0.6),
                                 nn.Linear(128, 1))
    ...
    def forward(self, f1_norm, f2_norm):
        ...
        for i in range(f1_norm.shape[0]):
            ...
            aud_vis_fts = torch.cat((audfts, visfts), 1)
            a_t = self.affine_a(aud_vis_fts.transpose(0,1))
            att_aud = torch.mm(audfts.transpose(0,1), a_t.transpose(0,1))
            ...
            att_vis = torch.mm(visfts.transpose(0,1), v_t.transpose(0,1))
            vis_att = self.tanh(torch.div(att_vis, math.sqrt(aud_vis_fts.shape[1])))

            H_a = self.relu(self.W_ca(audio_att) + self.W_a(audfts.transpose(0,1)))
            H_v = self.relu(self.W_cv(vis_att) + self.W_v(visfts.transpose(0,1)))

            att_audio_features = self.W_ha(H_a).transpose(0,1) + audfts
            att_visual_features = self.W_hv(H_v).transpose(0,1) + visfts
